...
Renders editable form fields dynamically based on extracted JSON data or dynamic schema definitions.
No hardcoded fields.
"""
...
def render_dynamic_form(data: Dict[str, Any], key_prefix: str = "field") -> Dict[str, Any]:
    ...
    Args:
        data: Extracted key-value dictionary.
        key_prefix: Unique key prefix for Streamlit state management.

    Returns:
        Dict[str, Any]: Modified/verified form data.
    """
    edited_data = {}
        ...
        if isinstance(value, dict):
            ...
            with st.expander(f"📋 {label} ({len(value)} items)", expanded=True):
                ...
                for idx, item in enumerate(value):
                    if isinstance(item, dict):
                        ...
                        edited_item = render_dynamic_form(
                            item, key_prefix=f"{unique_key}_{idx}"
                        )
            ...
            if isinstance(value, int):
                edited_data[key] = st.number_input(
                    label, value=int(value), step=1, key=unique_key
                )
            else:
                edited_data[key] = st.number_input(
